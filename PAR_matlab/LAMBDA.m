function [afixed,sqnorm,Ps,Qzhat,Z,nfixed,mu, r_Z]=LAMBDA(ahat,Qahat,method,varargin)
%
...
if ~isequal(Qahat-Qahat'<1E-6,ones(size(Qahat)));
    disp ('Variance-covariance matrix is not symmetric!');
    Qahat
    pause
end;
...
switch method
    ...
    case 5 
        %%
        %[zpar,sqnorm,Qzparhat,Zpar,Ps,nfixed,zfixed] = parsearch(zhat,Qzhat,Z,L,D,P0,ncands);
        [sqnorm,Ps,nfixed,zfixed, iZt, delete_index_in_origin] = myparsearch(ahat,Qahat, record, zhat,Qzhat,Z,L,D,P0, ncands);
        if nfixed == 0 
            ncands = 1;  
        end
    case 6
...
afixed = iZt*zfixed; 
% partial fixed
if(~isempty(delete_index_in_origin))
    r_Z = zeros(length(ahat), length(ahat));
    n = 1;
    for i=1:length(delete_index_in_origin)
%   generate a row transformation matrix to move the unfixed ambiguities to front, as well as corresponding convariance matrix
        r_Z(n, delete_index_in_origin(i)) = 1;
        n = n+1;
    end
    for i=1:length(ahat)
        if(isempty(find(delete_index_in_origin==i)))
            r_Z(n, i) = 1;
            n = n+1;
        end
    end
    ...
    incrnew = r_Z*incr;
%     get the fixed ambiguities (at back)
    incrnew = incrnew(length(delete_index_in_origin)+1:end);
    afixed = afixed + repmat(incrnew,1,ncands);
else
    r_Z = eye(n);
