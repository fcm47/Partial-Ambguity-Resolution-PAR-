function [Qzhat,Z,L,D,zhat,iZt, record] = decorrel (Qahat,ahat)
...
%   record: record indices for ahat and Qahat
% ----------------------------------------------------------------------
% Function.: decorrel
...
if ~isequal(Qahat-Qahat'<1E-5,ones(size(Qahat)));
...
[L,D] = ldldecom (Qahat);
% [val, ind] = max(D);
% L1 = [L(1:ind-1, 1:ind-1),L(1:ind-1, ind+1:end)
%     L(ind+1:end, 1:ind-1),L(ind+1:end, ind+1:end)];
% D1 = [D(1:ind-1), D(ind+1:end)];
% ahat1 = [ahat(1:ind-1); ahat(ind+1:end)];
% ahat = ahat1;
% Qahat = L1'*diag(D1)*L1;
% [L,D] = ldldecom (Qahat);
...
i1   = n - 1;
sw   = 1;
record = 1:n; %record indices for ahat and Qahat
% ------------------------------------------
...
% r_Z = inv(r_Zt'); % 单位正交矩阵，转置矩阵等于逆矩阵，转置后再求逆，又变为本身
% s = 1:n;
% s = s';
% in = r_Z*s;
return;
